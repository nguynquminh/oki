    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          titleColor: "#3d3d3d",
          bodyColor: "#6b6b6b",
          borderColor: "rgba(255,133,169,0.3)",
          borderWidth: 1,
          cornerRadius: 12,
          padding: 14,
          displayColors: true,
      ...
      scales: {
        y: {
          beginAtZero: true,
          grid: { color: "rgba(255,133,169,0.06)", drawBorder: false },
          ticks: {
...
function renderMetrics(data) {
  ...
  if (commandsEl)
    commandsEl.textContent = data.totalCommands.toLocaleString("vi-VN");

  const guildsTrend = document.getElementById("stat-guilds-trend");
  if (guildsTrend)
    guildsTrend.innerHTML = `<i class="fas fa-arrow-up"></i> +${data.newGuildsThisMonth}`;

  const guildsSub = document.getElementById("stat-guilds-sub");
  if (guildsSub)
    guildsSub.textContent = `+${data.newGuildsThisMonth} servers trong tháng này`;

  const membersEl = document.getElementById("stat-total-members");
  if (membersEl)
    membersEl.textContent = data.totalMembers.toLocaleString("vi-VN");

  const membersTrend = document.getElementById("stat-members-trend");
  if (membersTrend)
    membersTrend.innerHTML = `<i class="fas fa-arrow-up"></i> +${(data.newMembersThisWeek / 1000).toFixed(1)}k`;

  const membersSub = document.getElementById("stat-members-sub");
  if (membersSub)
    membersSub.textContent = `Tăng ${data.memberGrowthPercent}% so với tuần trước`;
...
function renderServerStatus(guilds) {
  ...
  container.innerHTML = guilds
    .map((g) => {
      const statusLabel =
        { online: "Online", offline: "Offline", idle: "Idle" }[g.status] ??
        g.status;
      const subText =
        g.status === "offline"
...
setInterval(() => {
  if (liveIndicator)
    liveIndicator.style.opacity =
      liveIndicator.style.opacity === "0.4" ? "1" : "0.4";
}, 1200);
